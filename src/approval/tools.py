...
from approval.memory import get_memory_dir
...
# 这行代码定义了一个 类型别名（Type Alias）。它的含义：从现在起，代码里的 ToolDef 就完全等同于 dict（字典）。
# 为什么要这么写：
# 可读性：看到 dict 只知道是个字典，但看到 ToolDef，你就立刻明白这个字典是用来“定义工具（Tool Definition）”的。
# 易于维护：如果你以后想把这个类型改得更复杂（例如改为 TypedDict 以便精确限制 Key 的名称），你只需要修改这一行，而不需要去代码里到处改。

# ─── Tool definitions ───────────────────────────────────────
# list[ToolDef] (泛型列表) 使用了 Python 3.9+ 引入的 泛型（Generics） 语法。
...
# 这段代码实现了一个“延迟工具激活（Deferred Tool Activation）”机制。
# 它的核心逻辑是：有些工具（Tools）默认是隐藏的，只有当它们被“激活”后，才会包含在发送给 AI 的 API 请求中。这通常是为了节省 Token，或者为了安全起见，只在特定条件下才把高级工具暴露给 AI。
# _activated_tools 这是一个集合（set），专门用来存储已经激活的工具名称。
...
# 这个函数负责筛选出当前真正要发给 AI API 的工具定义。
# 入参：可以传入一个工具列表，如果不传，默认使用全局的 tool_definitions。
# 筛选过滤规则 (if 分支)：
#     一个工具要被选中，必须满足以下两个条件之一：
#         not t.get("deferred")：该工具没有设置延迟加载（即它是常规工具，默认可见）。
#         t["name"] in _activated_tools：虽然它是延迟工具，但它的名字已经在已激活列表里了。
# 清理数据 ({k: v for ... if k != "deferred"})：
#     在返回字典时，把 deferred: True 这个键值对删掉。因为 Anthropic 或 OpenAI 的 API 不认识这个自定义字段，发过去会报错。
...
# 作用：找出那些“还没被激活”的延迟工具的名字。
# 用途：系统可以用这个列表来判断：如果 AI 想要执行某个操作但找不到工具，是否是因为该工具还没被激活？如果是，系统可以根据逻辑决定是否激活它。
...
# 读取一个文件的内容，并为每一行加上“行号”前缀。这种处理方式在 AI 辅助编程中非常常见，因为有了行号，AI 才能精准地告诉你：“我建议修改第 42 行”。
...
# 超长的文本结果进行“中间截断”处理。保留文本的开头和结尾，中间部分用一段提示文字替换
